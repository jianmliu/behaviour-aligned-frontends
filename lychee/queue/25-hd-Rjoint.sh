#!/usr/bin/env bash
# 噪声篇 25：R-joint 终评——联合训练 RNNoise 增强 → 物化+推理
set -euo pipefail
export WORK=/ephemeral/work
CK=$WORK/exp/rnj/r0/epoch_002.pt
...
python -c "import torch; torch.save(torch.load('$CK', map_location='cpu', weights_only=True)['fe'], '$WORK/exp/rnj/r0/fe_final.pt')"
...
CUDA_VISIBLE_DEVICES=0 python -u $WORK/nscripts/humdial_prep.py --humdial $WORK/humdial/en_test_nondev \
  ...
  --rnnoise $WORK/exp/rnj/r0/fe_final.pt --out $WORK/nz/hd_Rjoint --per-scen 100
pids=()
for i in $(seq 0 $((NG-1))); do
  CUDA_VISIBLE_DEVICES=$i python -u scripts/fdb_infer_lychee.py \
    --fdb_root $WORK/nz/hd_Rjoint --lychee_repo $WORK/third_party/Lychee-FD \
    --model_path $WORK/models/lychee-fd/lychee_full_duplex \
    --out_root $WORK/nz/ev_Rjoint --shard $i --num-shards $NG \
    > $WORK/logs/nz_Rj_$i.log 2>&1 & pids+=($!)
done
for p in "${pids[@]}"; do wait "$p" || true; done
n=$(set +o pipefail; ls $WORK/nz/ev_Rjoint/*/*/events.json 2>/dev/null | wc -l)
echo "ev_Rjoint 完成 $n 条"
[ "$n" -ge 950 ]
