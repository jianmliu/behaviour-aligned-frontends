...
model's turn decision through gradients, it reverses that effect, with leverage that tracks the acoustic
share of the failure and no cost in first-response relevance. (2)~The measurement instrument: a
...
\caption{Freeze-Omni, four systems on 1{,}000 paired samples per SNR. The signal-trained suppressor makes the eager model take over earlier at every SNR; the behaviour-aligned front-end, same parameters, makes it later, and its gain grows with the acoustic share. Dotted: clean audio.}
\label{fig:fo}
\end{figure}
...
\textbf{Use of AI.} Claude (Anthropic) assisted with code, experiment orchestration and editing; every result and claim was checked by the author, who takes full responsibility for them.
