...
front-end whose job is to serve the model's \emph{turn-taking decision}. We ask whether the deployed
remedy for noise---a suppressor trained on signal metrics---serves that decision, and what changes when
the same 78\,k-parameter RNNoise-style front-end is instead aligned to the decision through a frozen
model. As the measurement instrument we add a controlled noise protocol to the HumDial test set and
score first-response placement. On \lfd\ (12.8\,B), signal-trained suppression restores audibility and
with it every clean-condition pathology, while behaviour-level training cuts wrong-place responses from
67\,\% to 48\,\% ($p<10^{-23}$, paired) at no cost in on-time responses, staying silent where noise hides
the evidence. On Freeze-Omni (7B), an eager
model whose failures are linguistic, a front-end has no leverage at moderate noise; once masking is severe
enough to make noise a trigger, the frozen suppressor makes the model take over earlier and the
decision-aligned one later ($p<10^{-35}$), with first-response relevance preserved. Signal restoration is
not behaviour restoration: the two objectives select different front-ends, and the behaviour-aligned one's
leverage tracks the acoustic share of the failure. Aligning from the model's decisions alone, without
gradients, did not reach that solution at the budgets we tried. Protocol, metrics and recipes are released.
\end{abstract}
\begin{keywords}
...
\caption{Freeze-Omni, four systems, 1{,}000 paired samples per SNR. The signal-trained suppressor makes the eager model earlier at every SNR; the behaviour-aligned one (same parameters) is later than it at every SNR and later than no front-end from $-$10\,dB down. Dotted: clean.}
\label{fig:fo}
\end{figure}
