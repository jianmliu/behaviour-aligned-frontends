...
clean-condition pathology, while behaviour-level training cuts wrong-place responses from 67\,\% to
48\,\% ($p<10^{-23}$) without losing on-time responses, staying silent where noise hides the evidence.
On Freeze-Omni (7B) a front-end has no leverage at moderate noise; under severe masking the frozen
suppressor makes the model take over earlier and the decision-aligned one later ($p<10^{-35}$), with
first-response relevance preserved. Decision-only search without gradients did not find that direction.
...
held frozen throughout; whether such alignment is reachable without gradients, as a hosted API would
require, we only probe.
...
\caption{Freeze-Omni, 1{,}000 paired samples per SNR: no front-end, frozen signal-trained suppressor, the same front-end aligned through gradients, and ES on decisions. Aligned is later than frozen at every SNR, later than none from $-$10\,dB down. Dotted: clean.}
\label{fig:fo}
\end{figure}
...
measurement-only); two noise corpora (DEMAND, MUSAN), each held out between pretraining and test, and a front-end must be trained on its target noise; one LLM judge, opening sentences only; no signal-retrained control at the harsh SNRs; the gradient-free probe is zeroth-order search at small budgets on one model.
