...
them at zero marginal cost is a cost control, not an audio-quality option. Unlike enhancement-for-ASR joint training, the target is behaviour, not transcription.
...
measurement-only); two noise corpora (DEMAND, MUSAN), each held out between pretraining and test, and a front-end must be trained on its target noise; one LLM judge, opening sentences only; matched-SNR controls on Freeze-Omni only, DeepFilterNet3 on DEMAND only; the gradient-free probe is zeroth-order search at small budgets on one model.
...
Claude (Anthropic) drafted the code, job scripts, figure code and the text of all sections, and ran the experiment queues; the author designed the study, checked every number and claim, and takes full responsibility. No external funding.
