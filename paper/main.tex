...
67\,\% to 48\,\% ($p<10^{-23}$, paired) at no cost in on-time responses. On Freeze-Omni (7B), an eager
...
pathologies (\lfd) or makes it take over earlier (Freeze-Omni, at every SNR tried); aligned to the
...
($p<10^{-23}$, paired) at no cost in on-time responses, and ablations show the signal and behaviour
...
front-end from the same signal-pretrained start with a differentiable \emph{expected placement cost}
...
measurement-only); two noise corpora (DEMAND, MUSAN), each held out between pretraining and test, and a front-end must be trained on its target noise; one LLM judge, opening sentences only; the gradient-free probe is zeroth-order search at small budgets on one model.
...
alignment reverses the frozen suppressor's effect once masking creates an acoustic failure. This reframes the front-end
