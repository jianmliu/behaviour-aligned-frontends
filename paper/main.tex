...
measurement-only); two noise corpora (DEMAND, MUSAN), each held out between pretraining and test, and a front-end must be trained on its target noise; one LLM judge, opening sentences only; matched-SNR signal control on DEMAND only; the gradient-free probe is zeroth-order search at small budgets on one model.
