...
1{,}000 samples each) with still no misses; sustained masking acts as a trigger, whereas 300\,ms bursts at +10/+20\,dB barely register (42.5/40.0, 39.5/43.5): transient false activations are not this model's failure mode. Two
consequences follow. Self-distillation cannot be the target here---the model's own clean first
response precedes the annotated turn end by a median 7.3\,s on Full-Duplex-Bench---so we take the
benchmark's turn-end annotations as placement targets; and the front-end's job is restraint rather than restoration, so we also check clean audio,
where the front-ends leave placement unchanged (200 samples, n.s.). Under noise, the frozen suppressor nudges
...
measurement-only); single noise corpus (DEMAND), though held out between pretraining and test; one LLM judge, opening sentences only; the gradient-free probe is zeroth-order search at small budgets on one model.
