...
for sc, w in samples:
    ...
    od = a.out / sc / sid; oj = od / "events.json"
    if oj.exists():
        skip += 1; continue
    x = rd(w)
    ...
    for i in range(0, n, C):
        ...
        if s not in ("cl", "sl"):
            out["stat"] = "cl"
    el = time.time() - t1
    events = [{"type": "response", "start_time": first, "end_time": -1}] if first is not None else []
    od.mkdir(parents=True, exist_ok=True)
    oj.write_text(json.dumps({"events": events, "stats": stats, "snr": snr,
                              "dur": len(x) / 16000, "sec_per_chunk": el / max(len(stats), 1)},
                             ensure_ascii=False), encoding="utf-8")
    done += 1
    if done % 10 == 0:
        print(f"  {done}/{len(samples)}  最近: {sc}/{sid} first={first} "
              f"{el/max(len(stats),1)*1000:.0f}ms/chunk", flush=True)
